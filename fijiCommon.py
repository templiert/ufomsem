...
def stackFromIms(ims):
    ...
    imp.setDimensions(1, 1, len(ims)) # these have to be timeframes for trackmate
    return imp
...
def initTrakem(path, nbLayers, mipmaps = False): #initialize a project
    ...
    while not success:
        try:
            ...
            IJ.log('Problem creating a dummy trakem project. Retrying...')
...
def exportFlat_get_stack(project,scaleFactor, bitDepth = 8, layers = [], roi = '', flipX=False):
    images = []
    layerset = project.getRootLayerSet()
    loader = project.getLoader()
    for l,layer in enumerate(layerset.getLayers()):
        if (layers ==[] ) or (l in layers):
            ...
            if bitDepth == 8:
                ...
                imp = loader.getFlatImage(layer,roiExport,scaleFactor, 0x7fffffff, ImagePlus.GRAY16, Patch, layer.getAll(Patch), True, Color.black, None)
            images.append(imp)
            IJ.log('Layer ' + str(l) + ' flat exported')
    im_stack = stackFromIms(images)
    return im_stack
...
def readufomsemParameters(MagCFolder):
    MagCParametersPath = findFilesFromTags(MagCFolder, ['ufomsem_Parameters'])[0]
